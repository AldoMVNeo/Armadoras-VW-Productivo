...
{	
	...
	f!=undefined && f["rows_counter_text"]!=undefined ? totrows_text=f["rows_counter_text"] : totrows_text="Registros visibles: ";//defines rows counter text
	f!=undefined && f["btn_reset"]==true ? btnreset=true : btnreset=false;//show/hides reset link
	f!=undefined && f["btn_reset_text"]!=undefined ? btnreset_text=f["btn_reset_text"] : btnreset_text="Restablecer";//defines reset text
	...
	if(displaynrows || btnreset || displayPaging || displayLoader)
	{
		...
		if(displayLoader)
		{
			...
			var loaddiv = createElm( "div",["id","load_"+id],["class","loader"],["style","color:white"] );
                        var imagen=document.createElement("img");
                        imagen.id="imagenCarga";
                        imagen.src = "/armadoras/neology/recursos/imagenes/cargando.gif";
			loaddiv.className = "loader";// for ie<=6
			loaddiv.style.display = "none";		
                        loaddiv.appendChild(imagen);
		...
		if(displayPaging)
		{
			...
			grabEBI("mdiv_"+id).appendChild( createText(" Página ") );
			grabEBI("mdiv_"+id).appendChild(slcPages);
			grabEBI("mdiv_"+id).appendChild( createText(" de ") );
